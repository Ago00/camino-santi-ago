...
**Actualización (2026-08-09, revisión de "Pantalla de llegada: tiempo/ritmo anclados al último GPS"):** la triplicación descrita arriba ya no existe. `ModoDurante.tsx` migró a las funciones compartidas de `lib/ritmo.ts` en DT-020; esta tarea elimina la última implementación privada restante (`formatearTiempoTotal` de `app/page.tsx`), migrando `ModoLlegadaConectado`/`ModoLlegadaLibreConectado` a `calcularTiempoEnMarchaIntento`/`calcularRitmoMedioIntento`. Con esto ya no hay "tres sitios" que blindar por separado — todo el cálculo de tiempo en marcha y ritmo medio del proyecto pasa por un único punto (`lib/ritmo.ts`). El problema de fondo (sin guard contra `NaN`) sigue sin resolver, solo que ahora arreglarlo en ese único fichero lo cierra en todas las pantallas a la vez.
**Prioridad:** Baja.
